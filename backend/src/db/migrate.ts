...
-- ── EMPRESAS DESTRAVA SINCRONIZADAS E COMENTÁRIOS AUDITÁVEIS ──
CREATE TABLE IF NOT EXISTS destrava_empresas_cache (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id UUID NOT NULL REFERENCES organizacoes(id) ON DELETE CASCADE,
  external_id TEXT NOT NULL, nome TEXT NOT NULL, documento TEXT, email TEXT, telefone TEXT, status TEXT,
...
CREATE TABLE IF NOT EXISTS tarefas_comentarios (
  ...
  org_id UUID NOT NULL REFERENCES organizacoes(id) ON DELETE CASCADE,
  tarefa_id UUID NOT NULL REFERENCES tarefas(id) ON DELETE CASCADE,
  checklist_id TEXT, autor_id UUID NOT NULL REFERENCES profiles(id) ON DELETE RESTRICT,
