...
// DELETE /api/auth/users/:id  (somente gestor)
router.delete('/users/:id', authMiddleware, async (req: Request, res: Response): Promise<void> => {
  try {
    if (req.user!.role !== 'gestor') {
      res.status(403).json({ error: 'Apenas gestores podem remover usuários.' })
      return
    }
    const { id } = req.params
    ...
    const perfil = await queryOne<{ id: string }>(
      'SELECT id FROM profiles WHERE id = $1 AND org_id = $2',
      [id, orgId]
    )
    if (!perfil) {
      res.status(404).json({ error: 'Usuário não encontrado nesta organização.' })
      return
    }

    // Remove refresh tokens, tarefas atribuídas ficam com responsavel_id mas sem acesso
