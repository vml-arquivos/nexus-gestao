...
function canDeleteFinancial(role: string | undefined): boolean {
...
function buildNaturalGroupKey(row: any): string {
  const pessoa =
    row.pessoa_id || row.pessoa_nome_atual || row.pessoa_nome || "sem-pessoa";
...
function normalizeBaseTitleForHistory(title: unknown): string {
  return String(title || "")
    .replace(
      /^(Abatimento|Acréscimo|Acrescimo|Pagamento|Baixa|Recalculo|Recálculo)\s+[—-]\s+/i,
      "",
    )
    .trim();
}
...
function historicoFromParcela(row: any) {
  ...
  return {
    ...
    titulo: row.num_parcela
      ? `Parcela ${row.num_parcela} paga`
      : "Pagamento registrado",
    descricao: row.obs || null,
    valor: Number(row.valor || 0),
    data_evento: normalizeDateValue(
      row.pago_em || row.vencimento || row.updated_at || row.created_at,
    ),
...
router.get("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const pagamentos = await PaymentService.listPayments(
      orgId,
      userId,
      filtros,
    );
...
router.get("/grupos", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const row of rows) {
      ...
      const chave = row.grupo_id
        ? `grupo:${row.grupo_id}`
        : buildNaturalGroupKey(row);
    ...
    for (const h of histRows) {
      ...
      if (h.pagamento_id && pagamentoIdToKey.has(String(h.pagamento_id)))
        keys.push(pagamentoIdToKey.get(String(h.pagamento_id))!);
      ...
      for (const key of keys) {
        ...
        g.historico.push({
          ...
          valor:
            h.valor !== null && h.valor !== undefined ? Number(h.valor) : null,
          data_evento: normalizeDateValue(h.data_evento),
          forma_pagamento: h.forma_pagamento || null,
          saldo_anterior:
            h.saldo_anterior !== null && h.saldo_anterior !== undefined
              ? Number(h.saldo_anterior)
              : null,
          saldo_posterior:
            h.saldo_posterior !== null && h.saldo_posterior !== undefined
              ? Number(h.saldo_posterior)
              : null,
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!titulo?.trim()) {
        res.status(400).json({ error: "Título do histórico é obrigatório." });
        return;
      }

      let finalGroupKey =
        group_key || (referencia ? buildHistoryGroupKey(referencia) : null);
      ...
      if (pagamento_id) {
        ...
        if (pagRef) {
          ...
          finalGroupKey = pagRef.grupo_id
            ? `grupo:${pagRef.grupo_id}`
            : buildNaturalGroupKey(pagRef);
        }
      }
      ...
      const row = await queryOne(
       ...
       RETURNING *`,
        [
          ...
          data_evento || null,
          forma_pagamento || null,
          saldo_anterior === undefined || saldo_anterior === ""
            ? null
            : Number(saldo_anterior),
          saldo_posterior === undefined || saldo_posterior === ""
            ? null
            : Number(saldo_posterior),
      ...
      res
        .status(500)
        .json({ error: "Erro ao registrar histórico financeiro." });
    }
  },
);
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    for (const date of buildRecurringDates(
      mainVencimento || undefined,
      recorrencia,
...
router.patch("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const pag = await PaymentService.updatePayment(
      req.params.id,
      orgId,
      userId,
      updates,
...
// Remove todos os lançamentos de um card financeiro agrupado.
// Aceita grupo_id UUID e chave natural. Esta rota PRECISA vir antes de /:id.
router.delete(
  "/grupo/:grupo_id",
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { orgId, role } = req.user!;
      if (!canDeleteFinancial(role)) {
        res
          .status(403)
          .json({
            error:
              "Apenas admin, dev ou gestor podem apagar grupos financeiros.",
          });
        return;
      }

      const rawGrupoId = decodeURIComponent(
      ...
      // Chave natural: localiza todos os registros que compõem o mesmo card.
      // O fallback por partes evita falha quando a URL vem encoded, com acento,
      // espaço diferente ou quando o valor da parcela mudou após abatimento.
      const rows = (await query(
        `SELECT p.*, COALESCE(pe.nome, p.pessoa_nome) AS pessoa_nome_atual
       FROM pagamentos p
       LEFT JOIN pessoas pe ON pe.id = p.pessoa_id AND pe.org_id = p.org_id
      ...
      if (ids.length === 0 && rawGrupoId.startsWith("natural|")) {
        const parts = rawGrupoId.split("|");
        const [_, tituloKey, tipoKey, categoriaKey, pessoaKey] = parts;
        const fallbackIds = rows
          .filter((row) => !isSystemFinancialMovement(row))
          .filter((row) => {
            const titulo = normalizeGroupPart(
              normalizeBaseTitleForHistory(row.titulo),
            );
            const tipo = normalizeGroupPart(row.tipo);
            const categoria = normalizeGroupPart(
              row.categoria || "sem-categoria",
            );
            const pessoa = normalizeGroupPart(
              row.pessoa_id ||
                row.pessoa_nome_atual ||
                row.pessoa_nome ||
                "sem-pessoa",
            );
            return (
      ...
      await query(
        "DELETE FROM pagamentos_historico WHERE org_id = $1 AND (group_key = $2 OR pagamento_id = ANY($3::uuid[]))",
        [orgId, rawGrupoId, ids],
      ).catch(() => {});
...
router.delete("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await query(
      "DELETE FROM pagamentos_historico WHERE org_id = $1 AND pagamento_id = $2",
      [orgId, req.params.id],
    ).catch(() => {});
