    ...
    try {
      ...
      if (role === "sub_gestor") {
        if (id !== userId && target.criado_por !== userId) {
          res
            .status(403)
            .json({ error: "Subgestor só edita seus comandados." });
          return;
        }
        if (requestedRole && requestedRole !== "membro") {
          res
            .status(403)
            .json({ error: "Subgestor não pode promover usuários." });
          return;
        }
      }
    ...
    try {
      ...
      if (id === userId) {
        res
          ...
          .json({
            error:
              "Use a tela de configurações para alterar sua própria senha.",
          });
        return;
      }
      if (role === "sub_gestor" && target.criado_por !== userId) {
        res
          .status(403)
          .json({ error: "Subgestor só redefine senha de seus comandados." });
        return;
      }
      if (target.role === "gestor") {
        res
          .status(403)
          .json({ error: "Não é possível resetar senha de gestor." });
        return;
      }
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Gestor pode apagar usuários da própria organização, mas nunca admin/dev.
      // Subgestor mantém regra restrita aos próprios comandados.
      if (role === "gestor") {
        if (target.role === "admin" || target.role === "dev") {
          res
            .status(403)
            .json({ error: "Gestor não pode apagar admin ou dev." });
          return;
        }
      } else if (!isAdminOrDev(role)) {
        if (
          target.role === "admin" ||
          target.role === "dev" ||
          target.role === "gestor" ||
          target.role === "sub_gestor"
        ) {
          res
            .status(403)
            .json({
              error: "Apenas admin, dev ou gestor podem apagar este usuário.",
            });
          return;
        }
        if (role === "sub_gestor" && target.criado_por !== userId) {
          res
            .status(403)
            .json({ error: "Subgestor só apaga seus comandados." });
          return;
        }
      }
      ...
      try {
        await query("DELETE FROM refresh_tokens WHERE user_id = $1", [id]);
        await query(
          "DELETE FROM notificacoes WHERE user_id = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query(
          "DELETE FROM equipes_membros WHERE user_id = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query("DELETE FROM equipes_membros WHERE profile_id = $1", [
          id,
        ]).catch(() => {});
        ...
        await query(
          "DELETE FROM tarefa_historico WHERE org_id = $1 AND usuario_id = $2",
          [orgId, id],
        ).catch(() => {});

        // Apaga tarefas criadas por ele ou atribuídas a ele.
        await query(
          "DELETE FROM tarefas WHERE org_id = $1 AND (criado_por = $2 OR responsavel_id = $2)",
          [orgId, id],
        ).catch(() => {});

        // Apaga dados privados do usuário removido.
        await query(
          "DELETE FROM documentos WHERE criado_por = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query(
          "DELETE FROM pagamentos_historico WHERE user_id = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query(
          "DELETE FROM pagamentos WHERE criado_por = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query(
          "DELETE FROM agenda WHERE criado_por = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});
        await query("DELETE FROM pessoas WHERE user_id = $1 AND org_id = $2", [
          id,
          orgId,
        ]).catch(() => {});
        ...
        await query(
          "DELETE FROM convites WHERE org_id = $1 AND criado_por = $2",
          [orgId, id],
        ).catch(() => {});
        await query(
          "UPDATE profiles SET criado_por = NULL WHERE criado_por = $1 AND org_id = $2",
          [id, orgId],
        ).catch(() => {});

        await query("DELETE FROM profiles WHERE id = $1 AND org_id = $2", [
          id,
          orgId,
        ]);
        await query("COMMIT");
