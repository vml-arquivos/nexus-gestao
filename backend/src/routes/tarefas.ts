...
async function userCanAccessTask(
  ...
  if (role === "membro")
    return task.responsavel_id === userId || task.criado_por === userId;
  if (role === "gestor")
    return task.criado_por === userId || task.responsavel_id === userId;
  if (role === "sub_gestor") {
    if (task.criado_por === userId || task.responsavel_id === userId)
      return true;
    if (!task.responsavel_id) return false;
    const resp = await queryOne(
      "SELECT id FROM profiles WHERE id = $1 AND criado_por = $2",
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const responsavel = await queryOne<{ nome: string }>(
    ...
    try {
      ...
      if (
        ...
        res
          ...
          .json({
            error:
              "Você só pode atualizar status de tarefas atribuídas ou criadas por você.",
          });
        return;
      }
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filename = String(anexo.arquivo_url || "")
        .split("/uploads/")
        .pop();
      if (filename) {
    ...
    for (const key of allowed) {
      if ((req.body as any)[key] !== undefined) {
        if (key === "responsavel_id") {
          ...
          params.push(resp.nome);
        } else if (key === "checklist") {
          sets.push(`checklist = $${idx++}`);
          params.push(normalizeChecklist((req.body as any)[key]));
        } else {
          sets.push(`${key} = $${idx++}`);
          params.push((req.body as any)[key] || null);
        }
      }
    }
    if (!sets.length) {
...
router.delete("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!canDeleteOrgRecord(role)) {
      if (role === "membro" && existing.criado_por !== userId) {
        res
          .status(403)
          .json({
            error: "Membro só exclui tarefas pessoais criadas por ele.",
          });
        return;
      }
      if (
    ...
    await query("DELETE FROM tarefas WHERE id = $1 AND org_id = $2", [
      req.params.id,
      orgId,
    ]);
    res.json({ ok: true });
