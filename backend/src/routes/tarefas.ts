...
async function enviarEventoDestrava(
  ...
  try {
    ...
    console.warn(
      "[TAREFAS] Falha ao enviar evento para Destrava:",
...
function normalizeChecklistDifficulty(
  value: unknown,
  fallback: ChecklistDifficulty = "nivel_3",
): ChecklistDifficulty {
  ...
  if (
    ...
    return "nivel_1";
  if (raw === "nivel_2" || raw === "nível_2" || raw === "n2" || raw === "2")
    return "nivel_2";
...
function pointsForDifficulty(
  value: unknown,
  fallback: ChecklistDifficulty = "nivel_3",
) {
  return CHECKLIST_DIFFICULTY_POINTS[
    normalizeChecklistDifficulty(value, fallback)
  ];
}
...
function calculateTaskComplexityPoints(input: {
  ...
  const text = normalizeTextForScore(
    [
      ...
      input.origem_tipo,
      input.origem_nome,
      typeof input.origem_payload === "string"
        ? input.origem_payload
        : JSON.stringify(input.origem_payload || {}),
  ...
  const rules: Array<[RegExp, number]> = [
    [
      /\b(cnpj|cartao cnpj|receita federal|situacao cadastral|qsa|socios?|cnae|contrato social|nire)\b/,
      8,
    ],
    [
      /\b(rating|bancario|banco|limite|relacionamento bancario|restri(?:c|ç)ao|scr|registrato|serasa|spc|protesto)\b/,
      13,
    ],
    [
      /\b(faturamento|receita|dre|balanco|balan(?:c|ç)ete|extrato|fluxo de caixa|endividamento|margem|lucro)\b/,
      15,
    ],
    [
      /\b(documenta(?:c|ç)ao|documentos?|certid(?:a|ã)o|comprovante|upload|anexo|pendencia)\b/,
      6,
    ],
    [
      /\b(analise|diagnostico|viabilidade|credito|simulacao|proposta|aprovar|aprovacao|contrato|garantia)\b/,
      12,
    ],
    [
      /\b(urgente|critico|critica|risco|vencido|atrasado|prazo final|prioridade alta)\b/,
      10,
    ],
  ];
  for (const [pattern, add] of rules) if (pattern.test(text)) points += add;

  if (String(input.origem_sistema || "").toLowerCase() === "destrava")
    points += 10;
...
function calculateChecklistItemPoints(
  item: {
    ...
    data?: string;
    pontuacao?: number;
    dificuldade?: ChecklistDifficulty | string;
  },
  task: any,
) {
  const dificuldade = normalizeChecklistDifficulty(
...
function rankingChecklistKey(
  ...
  if (rawMotivo.includes(":"))
    return rawMotivo.split(":").slice(1).join(":").trim() || rawMotivo;
...
function parseChecklistItems(
  ...
  const raw = (() => {
    ...
    if (typeof value === "string") {
      try {
        ...
        if (
          parsed &&
          typeof parsed === "object" &&
          Array.isArray((parsed as any).checklist)
        )
          return (parsed as any).checklist;
        return [];
      } catch {
        return [];
      }
    ...
    if (value && typeof value === "object") {
      ...
      if (Array.isArray((value as any).checklist))
        return (value as any).checklist;
    }
    return [];
  })();
  ...
  return raw
    .map((item: any, index: number) => {
      ...
      const rawDate = String(
        item?.data || item?.date || item?.prazo || "",
      ).slice(0, 10);
      ...
      const dificuldade = normalizeChecklistDifficulty(
        item?.dificuldade,
        scoreToDifficulty(item?.pontuacao ?? 3),
      );
      const pontuacao = normalizeChecklistScore(
        item?.pontuacao,
...
async function normalizeChecklistForOrg(
  value: unknown,
  orgId: string,
  userId: string,
  role: string,
  ...
  const ids = Array.from(
    new Set(items.map((i) => i.responsavel_id).filter(Boolean)),
  ) as string[];
  ...
  return JSON.stringify(
    items.map((item) => {
      ...
      return {
        ...item,
        responsavel_nome: profile?.nome || item.responsavel_nome,
      };
    }),
  );
}
...
function isTaskSurprise(task: any) {
  ...
  return Boolean(
    payload?.nexus_tarefa_surpresa ||
    payload?.tarefa_surpresa ||
    payload?.surpresa_tarefa,
  );
}
...
function normalizePontuacaoEscopo(value: unknown): PontuacaoEscopo {
  const raw = String(value || "")
    .trim()
    .toLowerCase();
  if (["tarefa", "task", "somente_tarefa", "apenas_tarefa"].includes(raw))
    return "tarefa";
  if (
    [
      "subtarefa",
...
function taskVisibleSurprisePoints(task: any) {
  ...
  const checklistPoints = pontuacaoIncluiSubtarefas(scope)
    ? parseChecklistItems(task?.checklist).reduce(
        (acc, item) => acc + calculateChecklistItemPoints(item, task),
        0,
      )
    : 0;
  return Math.max(0, taskPoints + checklistPoints);
}

function taskSurprisePoints(task: any) {
  return taskVisibleSurprisePoints(task);
}
...
function shouldHideTeamTaskContentFromUser(
  task: any,
...
function maskParentTaskContentForMember(
  task: any,
  user: NonNullable<Request["user"]>,
  checklist: any[],
) {
  ...
  return {
    ...
    obs: null,
    origem_payload: {
      ...parseOriginPayloadSafe(task?.origem_payload),
      conteudo_restrito_por_privacidade: true,
      pontos,
    },
  };
}
...
function filterChecklistForUser(task: any, user: NonNullable<Request["user"]>) {
  ...
  if (assignedToMe.length)
    return assignedToMe.map((item) =>
      maskSurpriseChecklistItem(item, userId, task),
    );
  ...
  if (!shouldRevealTaskToUser(task, user)) {
    ...
    return {
      ...
      origem_payload: {
        ...
        conteudo_restrito_por_surpresa: true,
        pontos: pts,
        nexus_pontuacao_escopo: taskPontuacaoEscopo(task),
      },
    };
  }
...
function checklistForUserProgress(task: any, userId: string) {
  const items = parseChecklistItems(task?.checklist).filter((item) =>
    isChecklistItemExecutor(task, item, userId),
  );
  const total = items.length;
  const feitos = items.filter((item) => !!item.feito).length;
  return { items, total, feitos, completo: total > 0 && feitos === total };
...
async function syncChecklistTable(input: {
  orgId: string;
  tarefaId: string;
  userId: string;
  checklist: unknown;
  ...
  try {
    ...
    console.warn(
      "[TAREFAS] Checklist auxiliar não sincronizado; JSON principal preservado:",
      err,
    );
  }
}
...
async function userCanAccessTask(
  ...
  if (role === "sub_gestor") {
    ...
    const resp = await queryOne(
      "SELECT id FROM profiles WHERE org_id = $1 AND id = $2 AND criado_por = $3",
      [orgId, task.responsavel_id, userId],
    );
    return !!resp;
  }
...
const rawTasksCache = new SingleFlightTtlCache<string, any[]>(
  RAW_TASKS_CACHE_TTL_MS,
  24,
);

async function fetchRawTasksForOrg(orgId: string): Promise<any[]> {
  return rawTasksCache.get(orgId, () =>
    query<any>(
      `SELECT t.*,
       ...
       LEFT JOIN profiles ap ON ap.id = t.aceita_por
       LEFT JOIN (
         SELECT a.org_id, a.tarefa_id, COUNT(*)::int AS anexos_count,
                MAX(a.created_at) AS ultima_evidencia_em
         FROM tarefa_anexos a
         WHERE a.org_id = $1
         GROUP BY a.org_id, a.tarefa_id
       ) anexos ON anexos.tarefa_id = t.id AND anexos.org_id = t.org_id
       WHERE t.org_id = $1
       ORDER BY COALESCE(t.data_reabertura, t.updated_at, t.created_at) DESC,
                t.created_at DESC`,
      [orgId],
    ),
  );
...
function taskMatchesMember(task: any, memberId: string) {
  if (isPersonalScope(task))
    return (
      task.criado_por === memberId ||
      task.responsavel_id === memberId ||
      hasChecklistAssignedTo(task, memberId)
    );
  return (
    task.responsavel_id === memberId ||
    task.criado_por === memberId ||
    task.aceita_por === memberId ||
    hasChecklistAssignedTo(task, memberId)
  );
}
...
function buildTaskStats(tasks: any[]) {
  const count = (statuses: string[]) =>
    tasks.filter((t) => statuses.includes(String(t.status || ""))).length;
  return {
...
router.get("/dashboard", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const tarefas = await listTasksForUser(req.user!);
    const isOpen = (t: any) =>
      !["concluida", "aprovada", "cancelada"].includes(String(t.status || ""));
    ...
    if (role === "membro") {
      const resumo = {
        ...
        concluidas: tarefas.filter((t) =>
          ["concluida", "aprovada"].includes(String(t.status || "")),
        ).length,
        hoje: tarefas.filter(
          (t) => String(t.prazo || "").slice(0, 10) === today && isOpen(t),
        ).length,
      };
      res.json({ resumo });
      return;
    ...
    const resumo = {
      ...
      pendentes: tarefas.filter((t) => t.status === "pendente").length,
      aguardando_aprovacao: tarefas.filter(
        (t) =>
          t.status === "concluida" &&
          (t.status_gestor || "aguardando") === "aguardando",
      ).length,
      nao_concluidas: tarefas.filter((t) => t.status === "nao_concluida")
        .length,
      devolvidas: tarefas.filter((t) => t.status === "devolvida").length,
...
router.get("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (typeof prioridade === "string" && prioridade && prioridade !== "todos")
      tarefas = tarefas.filter((t) => t.prioridade === prioridade);
    if (
      typeof responsavel_id === "string" &&
      responsavel_id &&
      role !== "membro"
    ) {
...
router.get("/ranking", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const rawPeriodo =
      typeof req.query.periodo === "string"
        ? req.query.periodo.trim().toLowerCase()
        : "todos";
    ...
    const touchMember = (
      ...
      const when =
        tarefa.aprovado_em ||
        tarefa.tarefa_aprovada_em ||
        tarefa.updated_at ||
        tarefa.tarefa_updated_at ||
        tarefa.created_at ||
      ...
      if (
        when &&
        (!entry.ultima_aprovacao ||
          new Date(when).getTime() > new Date(entry.ultima_aprovacao).getTime())
      ) {
    ...
    for (const row of pontuacoesRegistradas) {
      const dataBase =
        row.aprovado_em ||
        row.tarefa_aprovada_em ||
        row.tarefa_updated_at ||
        row.tarefa_created_at;
      if (!inPeriod(dataBase)) continue;
      const motivo = String(row.motivo || "");
      const isChecklistScore =
        motivo.startsWith("checklist_aprovado") ||
        (String(row.checklist_id || "").trim().length > 0 &&
          String(row.checklist_id) !== "__tarefa__");
      const items = parseChecklistItems(row.checklist);
      const key = isChecklistScore
        ? rankingChecklistKey(null, row.motivo, row.checklist_id)
        : "__tarefa__";
      const matched = isChecklistScore
        ? items.find(
            (item) =>
    ...
    for (const tarefa of tarefasExecutadas) {
      ...
      if (pontuacaoIncluiSubtarefas(scoreScope) && feitos.length) {
        for (const item of feitos) {
          ...
          touchMember(
            participante,
            calculateChecklistItemPoints(item, tarefa),
            {
              ...tarefa,
      ...
      if (participanteTarefa && pontosTarefa > 0) {
        touchMember(
          ...
          {
            ...tarefa,
            motivo: "Tarefa aprovada pelo gestor",
            aprovado_por: tarefa.aprovada_por || null,
          },
          false,
          "__tarefa__",
    ...
    const ranking = Array.from(rankingMap.values()).sort((a, b) => {
      ...
      if (
        Number(b.subtarefas_executadas || 0) !==
        Number(a.subtarefas_executadas || 0)
      )
        return (
          Number(b.subtarefas_executadas || 0) -
          Number(a.subtarefas_executadas || 0)
        );
      const bt = b.ultima_aprovacao
        ? new Date(b.ultima_aprovacao).getTime()
        : 0;
      const at = a.ultima_aprovacao
        ? new Date(a.ultima_aprovacao).getTime()
        : 0;
    ...
    res.json({
      ...
      resumo: {
        ...
        pontos: ranking.reduce(
          (acc, item) => acc + Number(item.pontos || 0),
          0,
        ),
        subtarefas_executadas: ranking.reduce(
          (acc, item) => acc + Number(item.subtarefas_executadas || 0),
          0,
        ),
        tarefas_executadas: ranking.reduce(
          (acc, item) => acc + Number(item.tarefas_executadas || 0),
          0,
        ),
      },
    });
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        ...
        res
          .status(400)
          .json({ error: "Tarefa finalizada não pode ser selecionada." });
        return;
      }
      ...
      await addHistorico({
        ...
        tarefaId: tarefa.id,
        userId,
        acao: "tarefa_livre_aceita",
        statusAnterior: existing.status,
        statusNovo: "em_progresso",
        observacao: `${profile?.nome || "Membro"} assumiu a tarefa.`,
      });
      if (existing.criado_por && existing.criado_por !== userId) {
        await criarNotificacao({
          ...
          titulo: "🙋 Tarefa selecionada",
          body: `${profile?.nome || "Um membro"} assumiu a tarefa "${existing.titulo}".`,
...
async function findOpenChecklistAssignedToUser(
  orgId: string,
  userId: string,
...
async function findOpenTaskAssignedToUser(
  orgId: string,
  userId: string,
  excludeTaskId?: string,
) {
...
router.post("/", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const modoDistribuicao = normalizeTaskDistribution(
      (req.body as any).modo_distribuicao,
    );
    const pontuacaoManual = Number((req.body as any).pontuacao || 0);
    const pontuacaoEscopo = normalizePontuacaoEscopo(
      (req.body as any).pontuacao_escopo ||
        (req.body as any).pontuacao_tipo ||
        parseOriginPayloadSafe(origem_payload)?.nexus_pontuacao_escopo,
    );
    ...
    const hasResponsavelField = Object.prototype.hasOwnProperty.call(
      req.body,
      "responsavel_id",
    );
    let responsavelId: string | null = hasResponsavelField
      ? responsavel_id || null
      : userId;
    ...
    if (role === "membro") {
      ...
      if (
        role === "sub_gestor" &&
        resp.id !== userId &&
        resp.criado_por !== userId
      ) {
        res
          .status(403)
          .json({
            error:
              "Sub-gestor só pode atribuir tarefas para si ou seus comandados diretos.",
          });
        return;
      }
    }
    ...
    const tarefa = await queryOne<any>(
      ...
      [
        orgId,
        userId,
        modoFinal === "livre_equipe" ? null : responsavelId,
        modoFinal === "livre_equipe" ? null : responsavel?.nome || null,
        tituloFinal,
        descricao || null,
        data || null,
        prazo || null,
        ...
        pontuacao,
        contaRanking,
        origem_sistema === "destrava" ? "destrava" : null,
        origem_tipo || null,
        origem_id || null,
        origem_nome || null,
        origem_url || null,
    ...
    try {
      ...
      if (!["em_progresso", "concluida", "nao_concluida"].includes(status)) {
        res
          ...
          .json({
            error:
              "Status permitido: em_progresso, concluida ou nao_concluida.",
          });
        return;
      }
      if (
        status === "nao_concluida" &&
        !String(motivo_nao_conclusao || resposta_membro || "").trim()
      ) {
        res
          .status(400)
          .json({
            error: "Motivo é obrigatório para marcar como não concluída.",
          });
        return;
      }
      ...
      const params: unknown[] = [
        status,
        status === "em_progresso"
          ? existing.status_gestor || "aguardando"
          : "aguardando",
      ];
      let idx = 3;

      if (status === "em_progresso")
        sets.push(`data_inicio = COALESCE(data_inicio, NOW())`);
      if (status === "concluida") {
        sets.push(`data_conclusao = NOW()`);
        sets.push(`observacao_conclusao = $${idx++}`);
        params.push(observacao_conclusao || resposta_membro || null);
        sets.push(`resposta_membro = $${idx++}`);
        params.push(resposta_membro || observacao_conclusao || null);
        sets.push(`resposta_status = 'concluida'`);
        sets.push(`resposta_obs = $${idx++}`);
        params.push(observacao_conclusao || resposta_membro || null);
        sets.push(`resposta_em = NOW()`);
      }
      if (status === "nao_concluida") {
        sets.push(`motivo_nao_conclusao = $${idx++}`);
        params.push(motivo_nao_conclusao || resposta_membro);
        sets.push(`resposta_membro = $${idx++}`);
        params.push(resposta_membro || motivo_nao_conclusao);
      ...
      const tarefa = await queryOne<any>(
        `UPDATE tarefas SET ${sets.join(", ")} WHERE id = $${idx++} AND org_id = $${idx} RETURNING *`,
        params,
      );
      await addHistorico({
        orgId,
      ...
      if (
        ...
        await criarNotificacao({
          ...
          body:
            status === "concluida"
              ? `A tarefa "${existing.titulo}" foi concluída.`
              : `A tarefa "${existing.titulo}" não foi concluída.`,
          referenciaId: id,
          referenciaTipo: "tarefa",
        }).catch(() => {});
      }
    ...
    try {
      ...
      if (role === "membro" && existing.criado_por !== userId) {
        res
          ...
          .json({
            error:
              "Somente gestor, criador ou responsável administrativo pode cobrar manualmente esta tarefa.",
          });
        return;
      }
      ...
      for (const destinatario of recipients) {
        await criarNotificacao({
          ...
          body:
            mensagem ||
            `${autor?.nome || "Gestor"} enviou um lembrete para a tarefa "${existing.titulo}". Verifique o prazo, execute sua parte ou regularize o andamento.`,
          referenciaId: existing.id,
          referenciaTipo: "tarefa",
        });
        enviados++;
      ...
      await addHistorico({
        orgId,
        tarefaId: existing.id,
        userId,
        acao: "lembrete_manual",
        statusAnterior: existing.status,
        statusNovo: existing.status,
        observacao:
          mensagem || `Lembrete enviado para ${enviados} destinatário(s).`,
      });
      ...
      res
        .status(500)
        .json({ error: "Erro ao enviar lembrete manual da tarefa." });
    }
  },
);
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const podeRegistrarParte =
        hasChecklistAssignedTo(existing, userId) ||
        existing.responsavel_id === userId ||
        (!existing.responsavel_id && existing.criado_por === userId);
      ...
      if (
        geral.completo &&
        !["concluida", "aprovada"].includes(String(baseTask.status))
      ) {
        tarefaAtualizada =
          (await queryOne<any>(
            `UPDATE tarefas SET
        ...
        ["pendente", "devolvida", "reenviada"].includes(String(baseTask.status))
      ) {
        tarefaAtualizada =
          (await queryOne<any>(
            `UPDATE tarefas SET
      ...
      await addHistorico({
        ...
        observacao:
          ...
          `Parte do executor enviada (${minhaParte.feitos || geral.feitos}/${minhaParte.total || geral.total}).`,
      });
      ...
      if (existing.criado_por && existing.criado_por !== userId) {
        ...
        await criarNotificacao({
          ...
          titulo: geral.completo
            ? "✅ Todas as partes foram concluídas"
            : "✅ Parte enviada pelo membro",
          body: geral.completo
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (existing.conta_ranking !== false) {
        ...
        const participanteTarefa =
          existing.aceita_por || existing.responsavel_id;
        const pontosTarefa = pontuacaoIncluiTarefa(scoreScope)
          ? Math.max(
              0,
        ...
        if (
          ...
          await query(
           ...
           ON CONFLICT (tarefa_id, usuario_id, motivo) DO UPDATE SET
            ...
            console.warn(
              "[TAREFAS] Falha ao registrar pontuação da tarefa:",
              (err as Error)?.message || err,
            ),
          );
        }
          ...
          for (const item of items) {
            ...
            await query(
             ...
             ON CONFLICT (tarefa_id, usuario_id, motivo) DO UPDATE SET
              ...
              console.warn(
                "[TAREFAS] Falha ao registrar pontuação de checklist:",
                (err as Error)?.message || err,
              ),
            );
          }
      ...
      if (existing.responsavel_id && existing.responsavel_id !== userId) {
        await criarNotificacao({
          ...
          body: `"${existing.titulo}" foi aprovada.`,
          referenciaId: req.params.id,
          referenciaTipo: "tarefa",
        }).catch(() => {});
      }
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const { orgId, userId, role } = req.user!;
      if (role === "membro") {
        res.status(403).json({ error: "Membro não devolve tarefa." });
        return;
      }
      const { ressalva_gestor } = req.body;
      if (!String(ressalva_gestor || "").trim()) {
        res.status(400).json({ error: "Ressalva é obrigatória." });
        return;
      }
      const existing = await getTaskForAccess(req.params.id, orgId);
      if (!existing) {
      ...
      await addHistorico({
        ...
        observacao: String(ressalva_gestor).trim(),
      });
      if (existing.responsavel_id && existing.responsavel_id !== userId) {
        await criarNotificacao({
          orgId,
          ...
          body: String(ressalva_gestor).trim(),
          referenciaId: req.params.id,
          referenciaTipo: "tarefa",
        }).catch(() => {});
      }
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (role === "membro") {
        res.status(403).json({ error: "Membro não pode reabrir tarefa." });
        return;
      }
      if (!canDeleteOrgRecords(role) && existing.criado_por !== userId) {
        res
          .status(403)
          .json({
            error: "Você só pode complementar tarefas criadas por você.",
          });
        return;
      }
      ...
      const novaObs = [existing.obs, obsComplemento]
        .filter(Boolean)
        .join("\n\n");
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (existing.status !== "devolvida") {
        res
          .status(400)
          .json({ error: "Somente tarefas devolvidas podem ser reenviadas." });
        return;
      }
...
router.post(
  ...
  async (req: MulterTaskRequest, res: Response): Promise<void> => {
    try {
      ...
      await enviarEventoDestrava(task, "tarefa.arquivo_enviado", {
        arquivo: {
          titulo: anexo?.titulo || null,
          nome_original: req.file.originalname || null,
          mime_type: req.file.mimetype || null,
          tamanho: req.file.size || null,
        },
        observacao: descricao ? String(descricao).trim() : null,
      });
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!anexo) {
        res.status(404).json({ error: "Arquivo da tarefa não encontrado." });
        return;
      }

      const filename = filenameFromUploadUrl(anexo.arquivo_url);
      if (!filename) {
        res.status(404).json({ error: "Arquivo físico não localizado." });
        return;
      }
      ...
      const originalName =
        path
          .basename(String(anexo.nome_original || anexo.titulo || filename))
          .replace(/[\r\n"]/g, "") || filename;
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const canDelete =
        canDeleteOrgRecords(role) ||
        anexo.enviado_por === userId ||
        task.criado_por === userId ||
        role === "sub_gestor";
      if (!canDelete) {
        res
          .status(403)
          .json({ error: "Você não tem permissão para excluir este anexo." });
        return;
      }
      ...
      await query(
        "DELETE FROM tarefa_anexos WHERE id = $1 AND tarefa_id = $2 AND org_id = $3",
        [req.params.anexoId, req.params.id, orgId],
      );
      await addHistorico({
        orgId,
        ...
        statusAnterior: task.status,
        statusNovo: task.status,
        observacao: anexo.titulo || anexo.nome_original || null,
      });
      res.json({ ok: true });
    } catch (err) {
  ...
  try {
    ...
    for (const key of allowed) {
      ...
      if (key === "prioridade") {
        setValue(
          "prioridade",
          normalizePriority((req.body as any)[key], existing.prioridade),
        );
        continue;
      }
      ...
      if (key === "titulo") {
        const nextTitulo =
          ...
          (normalizeTaskScope((req.body as any).escopo ?? existing.escopo) ===
          "equipe"
            ? "Tarefa da equipe"
            : "Tarefa pessoal");
      ...
      if (key === "pontuacao") {
        const nextScope = normalizePontuacaoEscopo(
          (req.body as any).pontuacao_escopo ||
            (req.body as any).pontuacao_tipo ||
            taskPontuacaoEscopo(existing),
        );
        setValue(
          "pontuacao",
          pontuacaoIncluiTarefa(nextScope)
            ? normalizePositiveScore(
                (req.body as any)[key],
                existing.pontuacao || 1,
              )
            : 0,
        );
        continue;
      ...
      if (key === "pontuacao_escopo" || key === "pontuacao_tipo") {
        const payload = parseOriginPayloadSafe(setMap.get("origem_payload")?.value ?? existing.origem_payload);
        const nextScope = normalizePontuacaoEscopo((req.body as any)[key]);
        payload.nexus_pontuacao_escopo = nextScope;
        setValue("origem_payload", JSON.stringify(payload));
        if (!pontuacaoIncluiTarefa(nextScope)) setValue("pontuacao", 0);
        else if ((req.body as any).pontuacao !== undefined)
          setValue(
            "pontuacao",
            normalizePositiveScore(
              (req.body as any).pontuacao,
              existing.pontuacao || 1,
            ),
          );
        continue;
      ...
      if (key === "checklist") {
        ...
        checklistStructureChanged =
          checklistStructureKey(existing.checklist) !==
          checklistStructureKey(nextChecklist);
        voltouParaExecucao =
          checklistStructureChanged &&
          statusShouldReturnToExecution(existing.status);
        ...
        if (voltouParaExecucao) {
          ...
          const nextScoreScope = normalizePontuacaoEscopo(
            (req.body as any).pontuacao_escopo ||
              (req.body as any).pontuacao_tipo ||
              taskPontuacaoEscopo(existing),
          );
          setValue(
            "pontuacao",
            normalizeTaskScope(existing.escopo) === "equipe" &&
              pontuacaoIncluiTarefa(nextScoreScope)
              ? normalizePositiveScore(
                  (req.body as any).pontuacao ?? existing.pontuacao,
                  existing.pontuacao || 1,
                )
              : 0,
          );
    ...
    const finalEscopo = normalizeTaskScope(
      setMap.get("escopo")?.value ?? existing.escopo,
    );
    ...
    if (voltouParaExecucao) {
      await addHistorico({
        ...
        observacao:
          "Checklist complementar adicionado; tarefa voltou para execução.",
      });
    ...
    if (
      ["22007", "22008", "23514", "22P02", "42703", "42710"].includes(pgCode)
    ) {
      res
        .status(400)
        .json({
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      req.body.status =
        resposta_status === "nao_concluida" ? "nao_concluida" : "concluida";
      req.body.motivo_nao_conclusao =
        resposta_status === "nao_concluida" ? resposta_obs : undefined;
      req.body.observacao_conclusao =
        resposta_status === "concluida" ? resposta_obs : undefined;
      ...
      if (status === "nao_concluida" && !String(resposta_obs || "").trim()) {
...
router.delete("/:id", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (role === "membro" && existing.criado_por !== userId) {
      res
        .status(403)
        .json({ error: "Membro só exclui tarefas pessoais criadas por ele." });
      return;
    }
    if (
      !canDeleteAny &&
    ...
    const anexos = await query<{ arquivo_url?: string }>(
