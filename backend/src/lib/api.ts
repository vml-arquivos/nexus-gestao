...
export const usersApi = {
  ...
  async update(id: string, payload: { nome?: string; cargo?: string; novoRole?: string; role?: string; ativo?: boolean }): Promise<UserProfile> {
  ...
  async uploadAvatar(id: string, file: File): Promise<UserProfile> {
    const form = new FormData()
    form.append('file', file)
    const data = await apiJson<{ user: UserProfile }>(`/users/${id}/avatar`, { method: 'POST', body: form })
    return data.user
  },
