...
ARG VITE_API_WRITE_TIMEOUT_MS=35000
ARG VITE_API_UPLOAD_TIMEOUT_MS=120000
ENV NPM_CONFIG_REGISTRY=https://registry.npmjs.org \
    NPM_CONFIG_FETCH_RETRIES=5 \
    NPM_CONFIG_FETCH_RETRY_MINTIMEOUT=10000 \
    NPM_CONFIG_FETCH_RETRY_MAXTIMEOUT=60000 \
    ...
    VITE_API_WRITE_TIMEOUT_MS=${VITE_API_WRITE_TIMEOUT_MS} \
    VITE_API_UPLOAD_TIMEOUT_MS=${VITE_API_UPLOAD_TIMEOUT_MS}
...
HEALTHCHECK --interval=15s --timeout=8s --start-period=60s --retries=10 CMD ["/bin/sh", "/app/healthcheck.sh"]
