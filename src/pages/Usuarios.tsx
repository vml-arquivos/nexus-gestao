...
function ModalUsuario({
  ...
  async function gerarConvite() {
    if (!email.trim()) {
      toast("Informe o e-mail para gerar convite.", "error");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    ...
    >
      <div className="modal-card" style={{ width: "min(100%, 520px)" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 13, color: "var(--text3)", marginTop: 4 }}>
              Crie acesso com senha provisória ou gere link de convite.
            </p>
          </div>
          <button
        ...
        <div className="form-grid" style={{ display: "grid", gap: 12 }}>
          ...
          <label className="form-group">
            ...
            >
              ...
              <option value="sub_gestor">Subgestor</option>
            </select>
          </label>
          <label className="form-group">
            <span className="form-label">Senha provisória opcional</span>
        ...
        >
          <button className="btn btn-primary" onClick={criar} disabled={saving}>
            {saving ? (
          ...
          <button
            ...
            disabled={saving}
          >
            <LinkIcon size={16} /> Criar convite
          </button>
        </div>
        ...
        {lastSenha && (
          ...
          >
            <b>Senha provisória:</b> <code>{lastSenha}</code>{" "}
            <button
              className="btn btn-ghost"
              onClick={() => copy(lastSenha)}
        ...
        {lastLink && (
          ...
          >
            ...
            >
              <Copy size={14} /> Copiar link para WhatsApp
            </button>
          </div>
        )}
      </div>
...
export default function Usuarios() {
  ...
  const canDeleteUsers =
    eu?.role === "admin" || eu?.role === "dev" || eu?.role === "gestor";
  ...
  async function resetar(id: string) {
    try {
      ...
      toast(e instanceof Error ? e.message : "Erro ao resetar senha.", "error");
    }
  }
  ...
  async function alternar(u: UserProfile) {
    try {
      ...
      setUsuarios((list) =>
        list.map((x) => (x.id === u.id ? { ...x, ativo: updated.ativo } : x)),
      );
    } catch (e) {
      toast(
  ...
  async function apagar(u: UserProfile) {
    ...
    try {
      ...
      toast(
        e instanceof Error ? e.message : "Erro ao apagar usuário.",
        "error",
      );
    }
  }
      ...
      >
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: "var(--text3)" }}>
            {roleLabel(u.role)} · {u.ativo === false ? "Inativo" : "Ativo"}
          </div>
        </div>
        {canManage && u.id !== eu?.id && (
          <div
            style={{
              display: "flex",
              gap: 8,
          ...
          >
            ...
            <button
              className="btn btn-ghost"
              title={u.ativo === false ? "Ativar usuário" : "Desativar usuário"}
              onClick={() => alternar(u)}
              style={{ padding: "8px 10px" }}
            >
              {u.ativo === false ? <Power size={15} /> : <PowerOff size={15} />}
            </button>
            {canDeleteUsers && (
              <button
                className="btn btn-ghost"
    ...
    <div className="page-container" style={{ maxWidth: 920 }}>
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text3)" }}>
            Crie membros, gere convites e gerencie acessos.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "grid", gap: 20 }}>
          <section>
            <h2 style={{ fontSize: 16, marginBottom: 10 }}>Gestão</h2>
            <div style={{ display: "grid", gap: 10 }}>
              {grupos.gestores.map(row)}
            </div>
          </section>
          <section>
            <h2 style={{ fontSize: 16, marginBottom: 10 }}>Membros</h2>
            <div style={{ display: "grid", gap: 10 }}>
              {grupos.membros.map(row)}
              {grupos.membros.length === 0 && (
