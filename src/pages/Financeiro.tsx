  ...
  return (
    <div className="form-group">
      ...
      <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 6 }}>
        Use esta opção para lançar quantas datas avulsas quiser para a mesma
        pessoa.
      </div>
    </div>
  );
      ...
      >
        ...
        >
          {jurosTotal > 0.01 && (
            <span>
              Juros:{" "}
              <strong style={{ color: "#F59E0B" }}>{fmt(jurosTotal)}</strong>
            </span>
          )}
          <span>
            Total:{" "}
            <strong style={{ color: "var(--text1)" }}>{fmt(totalFinal)}</strong>
          </span>
        </div>
      </div>
        ...
        {mostrar.map((d, i) => (
          <div
            ...
            style={{
              ...
              padding: "7px 14px",
              borderBottom:
                i < mostrar.length - 1 ? "1px solid var(--border)" : "none",
              fontSize: 13,
            }}
          >
            <span style={{ color: "var(--text3)", fontWeight: 500 }}>
              {i + 1}ª parcela — {fmtDate(d)}
            </span>
            <span
              style={{
                fontWeight: 800,
      ...
      {datas.length > 3 && (
        ...
        >
          {expanded ? (
            ...
            <>
              <ChevronDown size={13} /> Ver todas as {datas.length} parcelas
            </>
          )}
        </button>
      )}
...
function historicoDerivado(parcelas: Pagamento[]): HistoricoFinanceiroItem[] {
  ...
  for (const p of parcelas) {
    if (p.status === "pago") {
      eventos.push({
        ...
        titulo: p.num_parcela
          ? `Parcela ${p.num_parcela} paga`
  ...
  return eventos.sort(
    (a, b) =>
      new Date(b.created_at || b.data_evento || 0).getTime() -
      new Date(a.created_at || a.data_evento || 0).getTime(),
  );
}
...
function agruparLancamentosFinanceiros(pags: Pagamento[]): GrupoFinanceiro[] {
  ...
  return Array.from(map.entries())
    .map(([key, itens]) => {
      ...
      const principal =
        ordenados.find((p) => p.status === "pendente") || ordenados[0];
      const pendentes = itens.filter((p) => p.status === "pendente");
      const pagos = itens.filter((p) => p.status === "pago");
      const proxima = pendentes.sort((a, b) =>
        (a.vencimento || "").localeCompare(b.vencimento || ""),
      )[0];
      const isVencido = !!(
        proxima?.vencimento &&
        new Date(`${proxima.vencimento.slice(0, 10)}T00:00:00`) < hoje
      );
      ...
      const grupoId = key.startsWith("grupo:")
        ? key.replace("grupo:", "")
        : null;
      ...
      return {
        ...
        pessoaNome:
          principal.pessoa_nome || principal.pessoa_nome_atual || "Sem pessoa",
    ...
    .sort((a, b) => {
      ...
      return (a.proximoVencimento || "").localeCompare(
        b.proximoVencimento || "",
      );
    });
}
...
function GrupoCard({
  ...
  return (
    <div
      style={{
        background: "var(--bg2)",
        border: `1px solid ${g.isVencido ? "rgba(239,68,68,0.35)" : "var(--border)"}`,
        borderRadius: "var(--radius)",
        overflow: "hidden",
      }}
    >
      ...
      <div
        ...
        onClick={() => isGrupo && setExpanded((e) => !e)}
      >
        <div
          style={{
            display: "flex",
        ...
        >
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              ...
              {isGrupo && (
                ...
                >
                  {g.itens.filter((p) => p.status === "pago").length}/
            ...
            >
              ...
              {g.proximoVencimento && (
                <span
                  style={{
                    fontSize: 12,
                    color: g.isVencido ? "#EF4444" : "var(--text3)",
                    display: "flex",
                    alignItems: "center",
                    gap: 4,
                  }}
                >
                  ...
                  {g.isVencido ? "Venceu " : "Vence "}
                  {fmtDate(g.proximoVencimento)}
                </span>
              )}
            </div>
          </div>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            >
              {sinal}
              {fmt(isGrupo ? g.pendente : g.total)}
            </div>
            {isGrupo && g.pago > 0 && (
              <div
          ...
          <div style={{ marginTop: 10 }}>
            ...
            >
              <div
                style={{
                  height: "100%",
                  width: `${Math.min(100, (g.pago / g.total) * 100)}%`,
                  background: "#10B981",
                  borderRadius: 999,
            ...
            .map((p, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "10px 16px",
                  borderBottom:
                    i < g.itens.length - 1 ? "1px solid var(--border)" : "none",
                  background:
                    p.status === "pago"
                ...
                <div>
                  <div
                    style={{
                      ...
                      fontWeight: 600,
                      color:
                        p.status === "pago" ? "var(--text3)" : "var(--text1)",
                    }}
                  >
                    {i + 1}ª parcela
                    {p.status === "pago" && (
                    ...
                    {p.status === "pendente" &&
                      p.vencimento &&
                      new Date(`${p.vencimento.slice(0, 10)}T00:00:00`) <
                        new Date() && (
                        <span
                          style={{
                            marginLeft: 6,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span
                    style={{
                      fontWeight: 700,
                      fontSize: 13,
                      color: p.status === "pago" ? "#10B981" : valorColor,
                    }}
                  >
                    {fmt(Number(p.valor))}
                  </span>
...
function GerenciarDividaModal({
  ...
  const novaParcelaPMT =
    saldo.numPendentes > 0 ? novoSaldo / saldo.numPendentes : 0;
  ...
  useEffect(() => {
    ...
    const sugerido =
      saldo.pendentes[0]?.valor ||
      (saldo.numPendentes
        ? saldo.totalPendente / saldo.numPendentes
        : saldo.totalPendente);
    if (sugerido > 0)
      setValor(
        String(Math.round(Number(sugerido) * 100) / 100).replace(".", ","),
      );
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const eventoHistorico = await pagamentosApi.addHistorico({
        ...
        titulo:
          modo === "abatimento"
            ? "Abatimento / pagamento registrado"
            : "Acréscimo registrado",
      ...
      setHistoricoLocal((prev) => [
        eventoHistorico as HistoricoFinanceiroItem,
        ...prev,
      ]);
          ...
          if (modo === "abatimento") {
            ...
            for (const p of saldo.pendentes) {
              ...
              if (restante >= atual) {
                ...
                await pagamentosApi.update(p.id, {
                  valor: Math.round((atual - restante) * 100) / 100,
                });
                restante = 0;
              }
            }
            ...
            if (primeira)
              await pagamentosApi.update(primeira.id, {
                valor:
                  Math.round((Number(primeira.valor) + valorNum) * 100) / 100,
              });
          }
        }
      }
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 18 }}>
          {ref?.titulo}
          {ref?.pessoa_nome ? ` · ${ref.pessoa_nome}` : ""}
        </div>
            ...
            [
              {
                ...
                value: saldo.totalOriginal,
                color: "var(--text1)",
                bg: "var(--bg3)",
              },
              {
                label: "Já pago",
                value: saldo.totalPago,
                color: "#10B981",
                bg: "rgba(16,185,129,0.1)",
              },
              {
                label: "Saldo restante",
                value: saldo.totalPendente,
                color: "#EF4444",
        ...
        >
          {saldo.numPendentes} parcela{saldo.numPendentes !== 1 ? "s" : ""}{" "}
          pendente{saldo.numPendentes !== 1 ? "s" : ""} ·{" "}
          {fmt(saldo.totalPendente / (saldo.numPendentes || 1))} cada
        </div>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 11, color: "var(--text3)" }}>
                Todos os pagamentos, abatimentos e acréscimos deste registro.
              </div>
            </div>
            <span
              style={{
            ...
            >
              {historicoCompleto.length} evento(s)
            </span>
          </div>
              ...
              {historicoCompleto.map((h, i) => (
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 900, fontSize: 13 }}>
                        {h.titulo || tituloEventoFinanceiro(h.tipo_evento)}
                      </div>
                      <div
                        style={{
                          fontSize: 11,
                      ...
                      >
                        {h.data_evento
                          ? fmtDate(String(h.data_evento))
                          : fmtDateTime(h.created_at)}
                        {h.forma_pagamento ? ` · ${h.forma_pagamento}` : ""}
                      </div>
                    </div>
                  ...
                  {h.saldo_anterior !== null &&
                    h.saldo_anterior !== undefined &&
                    h.saldo_posterior !== null &&
                    h.saldo_posterior !== undefined && (
                      <div
                        style={{
                          fontSize: 11,
                          color: "var(--text3)",
                      ...
                      >
                        Saldo: {fmt(Number(h.saldo_anterior))} →{" "}
                        <strong>{fmt(Number(h.saldo_posterior))}</strong>
                      </div>
                    )}
                </div>
              ))}
        ...
        >
          <button
            ...
            style={{
              padding: "12px",
              borderRadius: "var(--radius)",
              border: `2px solid ${modo === "abatimento" ? "#10B981" : "var(--border)"}`,
              background:
                modo === "abatimento" ? "rgba(16,185,129,0.1)" : "var(--bg3)",
              cursor: "pointer",
              fontWeight: 700,
              fontSize: 13,
              color: modo === "abatimento" ? "#10B981" : "var(--text2)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 6,
          ...
          <button
            type="button"
            onClick={() => setModo("acrescimo")}
            style={{
              padding: "12px",
              borderRadius: "var(--radius)",
              border: `2px solid ${modo === "acrescimo" ? "#F59E0B" : "var(--border)"}`,
              background:
                modo === "acrescimo" ? "rgba(245,158,11,0.1)" : "var(--bg3)",
              cursor: "pointer",
              fontWeight: 700,
              fontSize: 13,
              color: modo === "acrescimo" ? "#F59E0B" : "var(--text2)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 6,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div
            style={{
              display: "grid",
              gridTemplateColumns: modo === "abatimento" ? "1fr 1fr" : "1fr",
              gap: 10,
            }}
          >
          ...
          <div className="form-group" style={{ margin: 0 }}>
            ...
            >
              <button
                type="button"
                className={`btn ${acao === "recalcular" ? "btn-primary" : "btn-ghost"}`}
                onClick={() => setAcao("recalcular")}
                style={{ fontSize: 12 }}
              >
                Recalcular todas
              </button>
              <button
                type="button"
                className={`btn ${acao === "proximas" ? "btn-primary" : "btn-ghost"}`}
                onClick={() => setAcao("proximas")}
                style={{ fontSize: 12 }}
              >
                Abater próximas
              </button>
            ...
            <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 6 }}>
              O sistema calcula automaticamente o novo saldo. Você escolhe se
              quer redistribuir nas parcelas restantes ou baixar nas próximas
              parcelas.
            </div>
          </div>
          ...
          {valorNum > 0 && saldo.numPendentes > 0 && (
            <div
              style={{
                borderRadius: 10,
                border: `1px solid ${quitado ? "rgba(16,185,129,0.4)" : modo === "acrescimo" ? "rgba(245,158,11,0.4)" : "rgba(99,102,241,0.3)"}`,
                background: quitado
              ...
              {!quitado ? (
                ...
                >
                  {[
                    ...
                    {
                      label:
                        modo === "abatimento" ? "− Pagamento" : "+ Acréscimo",
                      value: valorNum,
                      sign: modo === "abatimento" ? "−" : "+",
                      color: modo === "abatimento" ? "#10B981" : "#F59E0B",
                    },
                  ...
                  >
                    <span style={{ color: "var(--text3)" }}>
                      Nova parcela ({saldo.numPendentes}x restantes)
                    </span>
                ...
                <div style={{ fontSize: 12, color: "var(--text3)" }}>
                  As {saldo.numPendentes} parcelas pendentes serão canceladas
                  automaticamente.
                </div>
              )}
            </div>
...
function PagamentoModal({
  ...
  const [intervaloParc, setIntervaloParc] = useState<
    "mensal" | "quinzenal" | "semanal"
  >("mensal");

  const initialMode: ScheduleMode =
    initial?.recorrencia && initial.recorrencia !== "nenhum"
      ? "recorrente"
      : "unico";
  const [scheduleMode, setScheduleMode] = useState<ScheduleMode>(initialMode);
  const [recorrencia, setRecorrencia] = useState(
    initial?.recorrencia || "mensal",
  );
  const [recorrenciaFim, setRecorrenciaFim] = useState(
    initial?.recorrencia_fim?.slice(0, 10) || "",
  );
  ...
  async function handleSave() {
    ...
    if (scheduleMode === "unico" && !vencimento && !isEdit) {
      toast("Informe uma data ou escolha datas personalizadas", "error");
      return;
    }
    if (scheduleMode === "recorrente" && !vencimento && !isEdit) {
      toast("Informe a primeira data da recorrência", "error");
      return;
    }
    if (
      ...
      toast("Adicione pelo menos uma data personalizada", "error");
      return;
    }
    if (scheduleMode === "parcelado" && !vencimento) {
      toast("Informe a data da primeira parcela", "error");
      return;
    }
    if (scheduleMode === "parcelado" && numParcelas < 2) {
      toast("Mínimo de 2 parcelas", "error");
      return;
    }
    ...
    try {
      ...
      if (scheduleMode === "parcelado") {
        ...
        datasParcelado = gerarDatasParcelamento(
          vencimento,
          numParcelas,
          intervaloParc,
        );
      ...
      const obsComForma = [
        ...
        scheduleMode === "parcelado"
          ? `${numParcelas}x de ${fmt(valorFinal)}${parseFloat(taxaJuros) > 0 ? ` (${taxaJuros}% a.m.)` : ""}`
          : "",
        obs,
      ]
        .filter(Boolean)
      ...
      const payload: Partial<Pagamento> = {
        ...
        vencimento:
          scheduleMode === "personalizado"
            ? primeiraDataPersonalizada || undefined
        ...
        recorrencia_fim:
          scheduleMode === "recorrente" && recorrenciaFim
            ? recorrenciaFim
            : undefined,
    ...
    >
      ...
      >
        ...
        >
          <button
            ...
            style={{
              padding: "12px 16px",
              borderRadius: "var(--radius)",
              border: `2px solid ${tipo === "pagamento" ? "#EF4444" : "var(--border)"}`,
              background:
                tipo === "pagamento" ? "rgba(239,68,68,0.1)" : "var(--bg3)",
              cursor: "pointer",
              fontWeight: 700,
              fontSize: 14,
              color: tipo === "pagamento" ? "#EF4444" : "var(--text2)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 6,
          ...
          <button
            ...
            style={{
              padding: "12px 16px",
              borderRadius: "var(--radius)",
              border: `2px solid ${tipo === "recebimento" ? "#10B981" : "var(--border)"}`,
              background:
                tipo === "recebimento" ? "rgba(16,185,129,0.1)" : "var(--bg3)",
              cursor: "pointer",
              fontWeight: 700,
              fontSize: 14,
              color: tipo === "recebimento" ? "#10B981" : "var(--text2)",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              gap: 6,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div className="form-group">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <input
                className="form-input"
                style={{ flex: 1 }}
                placeholder="Ex: Consultoria, parcela, aluguel..."
                value={titulo}
                onChange={(e) => setTitulo(e.target.value)}
              />
              <MicBtn
                onResult={(t) => setTitulo((prev) => (prev + " " + t).trim())}
              />
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (e.target.value) setPessoaNome("");
              }}
            >
          ...
          {!pessoaId && (
            <div className="form-group">
              <label className="form-label">Nome da pessoa livre</label>
              <input
                className="form-input"
                placeholder="Nome sem cadastro..."
                value={pessoaNome}
          ...
          <div className="form-group">
            ...
            >
              <button
                type="button"
                onClick={() => setScheduleMode("unico")}
                className={`btn ${scheduleMode === "unico" ? "btn-primary" : "btn-ghost"}`}
                style={{ fontSize: 12 }}
              >
                <CalendarDays size={14} /> Único
              </button>
              <button
                type="button"
                onClick={() => setScheduleMode("recorrente")}
                className={`btn ${scheduleMode === "recorrente" ? "btn-primary" : "btn-ghost"}`}
                style={{ fontSize: 12 }}
              >
              ...
              <button
                ...
                className={`btn ${scheduleMode === "personalizado" ? "btn-primary" : "btn-ghost"}`}
                style={{ fontSize: 12 }}
              >
                <ListPlus size={14} /> Datas
              </button>
              <button
                type="button"
                onClick={() => setScheduleMode("parcelado")}
                className={`btn ${scheduleMode === "parcelado" ? "btn-primary" : "btn-ghost"}`}
                style={{ fontSize: 12 }}
              >
          ...
          {scheduleMode === "parcelado" && (
            <>
              ...
              >
                <div className="form-group">
                  ...
                  <input
                    ...
                    value={numParcelas}
                    onChange={(e) =>
                      setNumParcelas(Math.max(2, parseInt(e.target.value) || 2))
                    }
                  />
                </div>
                <div className="form-group">
              ...
              >
                <strong>Como funciona:</strong> O valor digitado acima é o{" "}
                <em>total da dívida</em>. O sistema calcula automaticamente o
                valor de cada parcela
                {parseFloat(taxaJuros) > 0
                  ? " com juros compostos (Tabela Price)"
                  : " sem juros"}{" "}
                e cria um lançamento por parcela no financeiro, cada um na sua
                data correta.
              </div>
            </>
          )}
          ...
          {scheduleMode === "personalizado" && (
            <DateListEditor
              dates={datasPersonalizadas}
              setDates={setDatasPersonalizadas}
            />
          )}
          ...
          <div className="form-group">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              />
              <MicBtn
                onResult={(t) => setObs((prev) => (prev + " " + t).trim())}
              />
            </div>
          </div>
        </div>
      ...
      <div onClick={onClick} style={{ cursor: "pointer" }}>
        ...
        >
          ...
          >
            ...
            >
              <WalletCards size={12} /> Eu devo
            </div>
            <div
              style={{
                fontWeight: 800,
                fontSize: 15,
                color: r.devo_pendente > 0 ? "#EF4444" : "var(--text3)",
                fontFamily: "var(--font-heading)",
              }}
            >
              {fmt(r.devo_pendente)}
          ...
          >
            ...
            >
              <CircleDollarSign size={12} /> Me devem
            </div>
            <div
              style={{
                fontWeight: 800,
                fontSize: 15,
                color: r.me_devem_pendente > 0 ? "#10B981" : "var(--text3)",
                fontFamily: "var(--font-heading)",
              }}
            >
...
function compareNullableDates(a: unknown, b: unknown): number {
  ...
  const ta = da
    ? new Date(`${da}T00:00:00`).getTime()
    : Number.MAX_SAFE_INTEGER;
  const tb = db
    ? new Date(`${db}T00:00:00`).getTime()
    : Number.MAX_SAFE_INTEGER;
  return ta - tb;
}
...
function buildFinancialNaturalKey(g: GrupoPagamento): string {
  ...
  const pessoa =
    ...
    first?.pessoa_id ||
    first?.pessoa_nome ||
    first?.pessoa_nome_atual ||
    "sem-pessoa";
  const valorUnitario = Number(
    first?.valor ??
      (g.num_parcelas
        ? Number(g.valor_total || 0) / Number(g.num_parcelas || 1)
        : g.valor_total || 0),
  ).toFixed(2);
...
function normalizarGruposFinanceiros(
  input: GrupoPagamento[],
): GrupoPagamento[] {
  const map = new Map<string, GrupoPagamento>();

  for (const grupo of input || []) {
    const key = grupo.grupo_id
      ? `grupo:${grupo.grupo_id}`
      : buildFinancialNaturalKey(grupo);
    ...
    if (!map.has(key)) {
      map.set(key, {
        ...
        parcelas_pagas: Number(
          grupo.parcelas_pagas ||
            parcelas.filter((p) => p.status === "pago").length,
        ),
        parcelas_pendentes: Number(
          grupo.parcelas_pendentes ||
    ...
    const todasParcelas = Array.from(parcelasExistentes.values()).sort(
      (a, b) => {
    ...
    const ultima = [...todasParcelas].sort((a, b) =>
      compareNullableDates(b.vencimento, a.vencimento),
    )[0];
    const hoje = new Date().toISOString().slice(0, 10);

    existente.parcelas = todasParcelas;
    existente.valor_total = todasParcelas
      .filter((p) => p.status !== "cancelado")
      .reduce((s, p) => s + Number(p.valor || 0), 0);
    existente.valor_pago = pagas.reduce((s, p) => s + Number(p.valor || 0), 0);
    existente.valor_pendente = pendentes.reduce(
    ...
    existente.vencido = pendentes.some(
      (p) => compareNullableDates(p.vencimento, hoje) < 0,
    );
  ...
  return Array.from(map.values())
    .map((g) => {
      ...
      const ultima = [...parcelas].sort((a, b) =>
        compareNullableDates(b.vencimento, a.vencimento),
      )[0];
      const hoje = new Date().toISOString().slice(0, 10);
...
function GrupoBetaCard({
  ...
  const valorColor = g.tipo === "recebimento" ? "#10B981" : "#EF4444";
  const sinal = g.tipo === "recebimento" ? "+" : "-";
  const chip = g.vencido
    ? "Vencido"
    : g.valor_pendente === 0
      ? "Pago"
      : "Pendente";
  const chipColor =
    chip === "Vencido" ? "#EF4444" : chip === "Pago" ? "#10B981" : "#F59E0B";
  const chipBg =
    chip === "Vencido"
      ? "rgba(239,68,68,0.12)"
      : chip === "Pago"
  ...
  return (
    <div
      style={{
        background: "var(--bg2)",
        border: `1px solid ${g.vencido ? "rgba(239,68,68,0.35)" : "var(--border)"}`,
        borderRadius: "var(--radius)",
        overflow: "hidden",
      }}
    >
      ...
      >
        ...
        >
            ...
            >
              ...
              {g.proxima_parcela && (
                <span
                  style={{
                    fontSize: 12,
                    color: g.vencido ? "#EF4444" : "var(--text3)",
                    display: "flex",
                    alignItems: "center",
                    gap: 4,
                  }}
                >
                  <CalendarDays size={11} /> {g.vencido ? "Venceu " : "Vence "}
                  {fmtDate(g.proxima_parcela)}
                </span>
              )}
            </div>
          </div>
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            ...
            >
              {sinal}
              {fmt(isGrupo ? g.valor_pendente : g.valor_total)}
            </div>
            {isGrupo && g.valor_pago > 0 && (
              <div
                style={{ fontSize: 11, color: "var(--text3)", marginTop: 2 }}
              >
                Pago: {fmt(g.valor_pago)}
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ marginTop: 10 }}>
            ...
            >
              <div
                style={{
                  height: "100%",
                  width: `${Math.min(100, (g.valor_pago / g.valor_total) * 100)}%`,
                  background: "#10B981",
                  borderRadius: 999,
                  transition: "width 0.3s",
                }}
            ...
            >
              ...
              <span>
                {Math.round((g.valor_pago / g.valor_total) * 100)}% pago
              </span>
            </div>
          </div>
        )}
      ...
      {isGrupo && expanded && (
        <div style={{ borderTop: "1px solid var(--border)" }}>
          {[...g.parcelas]
            .sort((a, b) => {
              const byNum =
                Number(a.num_parcela || 0) - Number(b.num_parcela || 0);
              if (byNum !== 0) return byNum;
              return compareNullableDates(a.vencimento, b.vencimento);
            })
            .map((p, i) => (
...
export default function Financeiro() {
  ...
  const { user } = useAuth();
  const canDeleteFinanceiro =
    user?.role === "admin" || user?.role === "dev" || user?.role === "gestor";
  ...
  const [gerenciarDivida, setGerenciarDivida] = useState<{
    parcelas: Pagamento[];
    tipo: "pagamento" | "recebimento";
    historico?: HistoricoFinanceiroItem[];
  } | null>(null);
  ...
  const [pessoaFiltro, setPessoaFiltro] = useState<ResumoPorPessoa | null>(
    null,
  );
  ...
  async function handleMarcarPago(p: Pagamento) {
    try {
      ...
      const eventoHistorico = await pagamentosApi
        .addHistorico({
          pagamento_id: p.id,
          grupo_id: p.grupo_id || null,
          tipo_evento: "pagamento",
          titulo: p.num_parcela
            ? `Parcela ${p.num_parcela} marcada como paga`
  ...
  async function handleDelete(id: string) {
    if (!canDeleteFinanceiro) {
      toast(
        "Apenas admin, dev ou gestor podem apagar registros financeiros.",
        "error",
      );
      return;
    }
    if (
      !confirm(
        "Apagar definitivamente este registro financeiro? Esta ação não pode ser desfeita.",
      )
    )
      return;
    try {
      await pagamentosApi.remove(id);
      load();
  ...
  async function handleDeleteGrupo(g: GrupoPagamento) {
    ...
    if (
      !confirm(
        `Apagar definitivamente este registro financeiro completo${total > 1 ? ` com ${total} parcelas` : ""}? Esta ação não pode ser desfeita.`,
      )
    )
      return;
    try {
      if (g.grupo_id) {
        ...
        await Promise.all(
          (g.parcelas || []).map((p) => pagamentosApi.remove(p.id)),
        );
      }
      load();
      toast("Registro financeiro apagado");
    ...
    return grupos.filter((g) => {
      ...
      if (search) {
        ...
        return (
          ...
          (g.pessoa_nome || "").toLowerCase().includes(q) ||
  ...
  return (
    <div
      style={{
        padding:
          "20px 20px calc(var(--bottom-nav-h, 62px) + env(safe-area-inset-bottom, 0px) + 24px)",
        maxWidth: 760,
        margin: "0 auto",
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text3)", fontSize: 13, marginTop: 2 }}>
            Pagamentos, recebimentos, recorrências e datas personalizadas
          </p>
        </div>
        <button
          className="btn btn-primary"
      ...
      {resumo && (
        ...
        >
          ...
          >
            ...
            >
              {fmt(resumo.receita_pendente)}
            </div>
            <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 2 }}>
              Recebido: {fmt(resumo.receita_paga)}
            </div>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              {fmt(resumo.despesa_pendente)}
            </div>
            <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 2 }}>
              Pago: {fmt(resumo.despesa_paga)}
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, color: "#F59E0B" }}>
              {vencidos.length} lançamento{vencidos.length > 1 ? "s" : ""}{" "}
              vencido{vencidos.length > 1 ? "s" : ""}
            </div>
            <div style={{ fontSize: 12, color: "var(--text3)" }}>
              Total:{" "}
              {fmt(vencidos.reduce((s, p) => s + Number(p.valor_pendente), 0))}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="tabs" style={{ marginBottom: 16 }}>
        ...
        >
          <Filter size={14} /> Lançamentos
        </button>
        <button
          className={`tab ${tab === "pessoas" ? "active" : ""}`}
          onClick={() => setTab("pessoas")}
        >
          <User size={14} /> Por pessoa ({porPessoa.length})
        </button>
      </div>
          ...
          {porPessoa.length === 0 ? (
            ...
            >
              ...
              <div style={{ fontWeight: 700 }}>
                Nenhum lançamento por pessoa
              </div>
              <div style={{ fontSize: 13, marginTop: 4 }}>
                Vincule lançamentos a pessoas para ver o resumo
              </div>
            </div>
          ) : (
            porPessoa.map((r) => (
        ...
        <>
          ...
          >
            ...
            <select
              ...
              onChange={(e) =>
                setFiltroTipo(
                  e.target.value as "todos" | "pagamento" | "recebimento",
                )
              }
            >
          ...
          {pessoaFiltro && (
            ...
            >
              ...
              <span style={{ fontSize: 13, fontWeight: 600, flex: 1 }}>
                Filtrado: {pessoaFiltro.pessoa_nome}
              </span>
              <button
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {gruposPagar.length} registro
                        {gruposPagar.length === 1 ? "" : "s"}
                      </span>
                    </div>
                    <button
                      className="btn btn-ghost"
                      style={{ fontSize: 12, padding: "5px 10px", gap: 4 }}
                      onClick={() => openLancamento({ tipo: "pagamento" })}
                    >
                      <Plus size={13} /> Novo
                    </button>
                  </div>
                    ...
                    >
                      {gruposPagar.map((g) => (
                        <GrupoBetaCard
                          key={
                            g.grupo_id ||
                            `${g.titulo}-${g.pessoa_nome}-${g.valor_total}`
                          }
                          g={g}
                          onGerenciar={(gp) =>
                            setGerenciarDivida({
                              parcelas: gp.parcelas,
                              tipo: gp.tipo,
                              historico: gp.historico || [],
                            })
                          }
                          onEdit={(p) => {
                            setPrefill(null);
                ...
                <div>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {gruposReceber.length} registro
                        {gruposReceber.length === 1 ? "" : "s"}
                      </span>
                    </div>
                    <button
                      className="btn btn-ghost"
                      style={{ fontSize: 12, padding: "5px 10px", gap: 4 }}
                      onClick={() => openLancamento({ tipo: "recebimento" })}
                    >
                      <Plus size={13} /> Novo
                    </button>
                  </div>
