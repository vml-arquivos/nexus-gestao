...
import { CheckCircle2, Calendar, DollarSign, Users, FileText, TrendingUp, TrendingDown, Clock, AlertTriangle, ArrowRight } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    const tarefasHoje = store.tarefas.filter(t => t.data === hoje).length
    ...
    const recebimentos = store.pagamentos.filter(p => p.tipo === 'recebimento' && p.status === 'pago')
      .reduce((a, b) => a + Number(b.valor), 0)
    const pagamentos = store.pagamentos.filter(p => p.tipo === 'pagamento' && p.status === 'pago')
      .reduce((a, b) => a + Number(b.valor), 0)
    ...
    const equipeAtiva = store.pessoas.filter(p => p.tipo === 'funcionario' || p.tipo === 'prestador')

    return { tarefasPendentes, tarefasHoje, tarefasConcluidas, eventosHoje, recebimentos, pagamentos, saldo, vencidos, equipeAtiva }
  }, [hoje])
  ...
  const docsRecentes = store.documentos.slice(0, 4)
    ...
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'radial-gradient(circle,rgba(108,59,255,0.2),transparent 70%)',
          pointerEvents: 'none'
        }} />
        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 2, fontWeight: 500 }}>
          {saudacao},
        </div>
        <div style={{ fontFamily: 'var(--font-heading)', fontSize: 22, fontWeight: 800, marginBottom: 14, letterSpacing: '-0.02em' }}>
          {nome} 👋
        </div>
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 800, color: metrics.saldo >= 0 ? 'var(--success)' : 'var(--danger)', lineHeight: 1 }}>
              {fmt(metrics.saldo)}
            </div>
            ...
            {metrics.tarefasConcluidas > 0 && (
              <div style={{ marginTop: 8 }}>
                <ProgressBar
                  ...
                  max={metrics.tarefasConcluidas + metrics.tarefasPendentes}
                />
                <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 4 }}>
                  {metrics.tarefasConcluidas} concluídas
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4 }}>
            {metrics.equipeAtiva.slice(0, 6).map(p => (
              <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, minWidth: 60 }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text2)', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 60 }}>
                  {p.nome.split(' ')[0]}
                </span>
              </div>
            ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {tarefasRecentes.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: 'var(--bg3)', borderRadius: 8 }}>
                  <div style={{ width: 8, height: 8, borderRadius: 4, background: t.prioridade === 'alta' ? 'var(--danger)' : t.prioridade === 'media' ? 'var(--warning)' : 'var(--success)', flexShrink: 0 }} />
                  <span style={{ flex: 1, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.titulo}</span>
                  {t.prazo && (
                  ...
                  {t.responsavel_nome && (
                    <Avatar name={t.responsavel_nome} size={22} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {proximosPagamentos.map(p => {
                ...
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', background: isVencido ? 'rgba(239,68,68,0.08)' : 'var(--bg3)', borderRadius: 8 }}>
        ...
        {docsRecentes.length > 0 && (
          <div className="card">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
                <FileText size={16} color="var(--secondary)" /> Documentos Recentes
              </div>
              <Link to="/documentos" style={{ fontSize: 12, color: 'var(--primary-light)', textDecoration: 'none' }}>Ver todos</Link>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
              {docsRecentes.map(d => (
                <a key={d.id} href={d.arquivo_url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <div style={{ padding: '10px', background: 'var(--bg3)', borderRadius: 8, border: '1px solid var(--border)' }}>
                    <div style={{ fontSize: 20, marginBottom: 6 }}>
                      {d.mime_type?.includes('pdf') ? '📄' : d.mime_type?.includes('image') ? '🖼️' : '📎'}
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--text)' }}>
                      {d.titulo}
                    </div>
                    {d.pessoa_nome && (
                      <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{d.pessoa_nome}</div>
                    )}
                  </div>
                </a>
              ))}
        ...
        {/* Empty state */}
        {metrics.tarefasPendentes === 0 && metrics.eventosHoje.length === 0 && store.pagamentos.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: '40px 20px' }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>🚀</div>
            <div style={{ fontFamily: 'var(--font-heading)', fontSize: 18, fontWeight: 700, marginBottom: 8 }}>
              Tudo pronto!
            </div>
            <p style={{ color: 'var(--text2)', fontSize: 14, lineHeight: 1.7, marginBottom: 20 }}>
              Comece adicionando sua equipe, criando tarefas ou registrando pagamentos.
            </p>
