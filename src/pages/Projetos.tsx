          ...
          {groups.map(group => {
            ...
            return <article key={group.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
              <div className="flex items-start justify-between gap-3">
                ...
                <Link to={`/tarefas?view=grafo&task=${encodeURIComponent(firstTask.id)}`} className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-violet-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-violet-700 active:scale-95"><GitBranch size={14} /> Abrir mapa</Link>
              </div>
              <div className="mt-4 space-y-2">
                {group.tasks.slice(0, 5).map(task => <Link key={task.id} to={`/tarefas?view=grafo&task=${encodeURIComponent(task.id)}`} className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 px-3 py-2 transition hover:border-violet-200 hover:bg-violet-50/60 dark:border-slate-800 dark:hover:border-violet-900 dark:hover:bg-violet-950/20"><span className="flex min-w-0 items-center gap-2"><ListChecks size={14} className="shrink-0 text-slate-400" /><span className="truncate text-xs font-medium text-slate-700 dark:text-slate-200">{task.titulo}</span></span><span className="flex shrink-0 items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400">{statusLabel(String(task.status))}<ArrowRight size={12} /></span></Link>)}
                {group.tasks.length > 5 && <Link to={`/tarefas?view=grafo&task=${encodeURIComponent(firstTask.id)}`} className="block px-3 pt-1 text-[11px] font-medium text-violet-600 dark:text-violet-300">+{group.tasks.length - 5} tarefa(s) no mapa</Link>}
              </div>
            </article>
          })}
        </section>
