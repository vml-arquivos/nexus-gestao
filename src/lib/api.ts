...
export const auth = {
  ...
  async register(payload: { nome: string; email: string; senha: string; role: 'gestor' | 'sub_gestor' | 'membro'; orgNome?: string }) {
    const data = await apiJson<{ user: UserProfile; accessToken: string; refreshToken: string }>(
...
export const usersApi = {
   ...
   */
  async create(payload: { nome: string; email: string; role: 'sub_gestor' | 'membro'; senha?: string }): Promise<{ user: UserProfile; senha: string }> {
