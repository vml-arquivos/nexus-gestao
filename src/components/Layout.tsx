...
export default function Layout() {
  ...
  const bottomMain = NAV.filter(n => {
    ...
    if (user.role === 'membro' && n.path === '/equipe') return false
      ...
      <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}>
        ...
        <nav style={{ flex: 1, padding: '10px 0', overflowY: 'auto' }}>
          {NAV.map(({ path, icon: Icon, label }) => {
            ...
            if (user.role === 'membro') {
              if (['/usuarios','/equipe','/financeiro','/pessoas','/documentos','/relatorios'].includes(path)) return null
            }
            return (
              <Link
                key={path}
            ...
            <div style={{
              ...
              {(() => {
                ...
                if (user) {
                  ...
                  if (user.role !== 'membro') actions.push({ path: '/financeiro', label: 'Pagamento', color: '#059669' })
                  // Pessoas/Contato: apenas gestores e subgestores
                  if (user.role !== 'membro') actions.push({ path: '/pessoas', label: 'Contato', color: '#D97706' })
                }
